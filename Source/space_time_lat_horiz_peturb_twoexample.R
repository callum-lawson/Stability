...
rplot <- function(zmu,pars,xmin=0,xmax=10,nx=10^4,...){
	...
	points(kmat,rep(0,length(kmat)),pch=c(16,21),col=cseq,cex=1.3)
...
lettlab <- function(i){
	mtext(bquote(bold((.(letters[i])))),side=3,line=0.5,xpd=T,adj=0.05,cex=mycex)
	}

addxlab <- function(xname){
	mtext(xname,side=1,line=1,las=1,xpd=T,cex=mycex)
	}

addtitlab <- function(titname){
	mtext(titname,side=3,line=3,las=1,xpd=T,cex=mycex)
	}

addylab <- function(yname){
	mtext(yname,side=2,line=1.5,las=3,xpd=T,cex=mycex)
	}
...
pars1 <- data.frame(b0=5,b1=1,b2=0,b3=-1,b4=0)
pars2 <- data.frame(b0=2.5,b1=1,b2=0,b3=-0.5,b4=0)
pars3 <- data.frame(b0=4.5,b1=1,b2=0,b3=-0.8,b4=-0.3)
  # 5,2,0,-1,-0.5
...
# setwd("C:/Users/Callum/Dropbox/NIOO/Grants/Humboldt")
# setwd("D:/Users/calluml/Dropbox/NIOO/Grants/Humboldt")

lineylim <- c(-5,5)
densylim <- c(0,0.6)

tiff(filename=paste0("humboldt_theory_illustration_",format(Sys.Date(),"%d%b%Y"),".tiff"),
  width=5, height=5, units="in", res=600)

par(mfrow=c(2,2),mar=c(1,1.5,3,1.5),oma=c(3,3,3,3),
	bty="l",xaxt="n",yaxt="n"
	)
rplot(zmu,pars1,ylim=lineylim)
lettlab(1)
addylab("Population growth rate")
addtitlab(expression(bold(additive)))
# rplot(zmu,pars2,ylim=lineylim)
# lettlab(2)
# addtitlab(expression(bold(weak~DD)))
rplot(zmu,pars3,ylim=lineylim)
lettlab(2)
addtitlab(expression(bold(interactive)))

bw <- 0.5
dplot(xmat1,bw=bw,ylim=densylim)
lettlab(3)
addylab("Probability density")
addxlab("Population density")
# dplot(xmat2,bw=bw,ylim=densylim)
# lettlab(5)
# addxlab("Population density")
dplot(xmat3,bw=bw,ylim=densylim)
lettlab(4)
addxlab("Population density")
